...
- **External mic audio (#4) — Phase 1 (capture + record)**: done — the device page's *Audio only (mic)* source (with mic picker) publishes audio-only; the control service models it as a `kind:'audio'` source on a `micN` path (recorded as its own clip, persisted, kept out of tiles/takes/audio-only pre-flight); the dashboard has an Audio sources panel (add/rename/remove, link-to-camera, **Listen** WHEP monitor) and routes audio devices to mic slots. The finalized session records each source's `kind`+`link`+`recordStartedAt`. `npm test` (32) covers the audio-source flow (mic path recorded, not takeable, linked, finalized). Phase 2 (per-section export audio routing — mix/replace + volumes, edited in the preview) is next. The individual mic clips are already separate files for post.
...
- **Session export (v1)** re-encodes to a fixed **1080p30** H.264 + AAC and uses **one camera per program segment** with the active camera's audio (optional crossfade dissolves at takes, but no other transitions, no audio mixing/ducking, no per-export resolution/bitrate choice). Cross-clip sync is **approximate** (sub-second), matching the preview player — not frame-accurate. ffmpeg is pulled from BtbN's rolling `latest` build (Windows/Linux) and evermeet (macOS, Intel — runs under Rosetta on Apple Silicon); only the Windows download path is verified.
...
- **[External mic (e.g. DJI wireless) audio in clips](https://github.com/openidle-dev/idle-stream/issues/4)** (#4) — **Phase 1 done** (capture + record + monitor; see [Build Status](#build-status)): audio-only device source, `kind:'audio'` slots, recorded as separate clips, dashboard panel + Listen monitor, link-to-camera. **Phase 2 (next):** per-section export audio routing — mix/replace + camera/mic volumes, edited by clicking sections in the preview (with live Web-Audio monitoring), pulled through to the rendered MP4.
