...
- **Recordings browser**: a modal lists captured files per camera (size + time, download links) and the switch-log sessions (with a `switches.json` download), fetched from the control service's read-only `/api/*` endpoints. The clip endpoint serves files **inline with HTTP Range** so they also stream in-browser.
