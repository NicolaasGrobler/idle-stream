...
### Device PWA (`phone-pwa/`)
The capture client (served at the `:8443` origin). It's the **device page**, not just a phone page — it serves phone cameras, **desktop/laptop webcams**, and **screen share**. (The directory and wire protocol keep the `phone` name internally; only the visible copy says "device".)
- **Onboarding page** (`setup.html`, linked from the landing): a one-time, platform-aware setup screen — a "Download certificate" button (serves `/rootCA.pem`) with the exact iOS vs Android trust steps, an "Open the camera" link, and a **QR code** of the device URL (generated offline via the vendored `vendor/qrcode.js`) so the operator can display this page and other phones scan to land on it. This makes the unavoidable per-device CA-trust step tap-through instead of hand-typed instructions.
- Landing: device name + **source** (Camera — front/back — or **Screen share**) → **Join** (one gesture acquires the source, requests wake lock, connects the control WebSocket, registers).
- **Capture source.** *Camera* uses `getUserMedia` (a phone camera or a desktop webcam). *Screen share* uses `getDisplayMedia({video, audio:true})` (the browser's share dialog offers the audio opt-in) so a laptop or Android-Chrome screen becomes a normal camera slot — published through the same WHIP→MediaMTX pipeline, forcing H.264 so recording stays copy-only. The screen option is **feature-detected** (`navigator.mediaDevices.getDisplayMedia`), so it appears on desktop browsers and Android Chrome but not on iOS (all iOS browsers are WebKit, which lacks the API). The browser's own "Stop sharing" ends the track → the device stops publishing and drops in the roster. *Audio only (mic)* captures mic-only (`getUserMedia({audio})`) with an **in-page mic picker** (`enumerateDevices`, so you pick the external/DJI mic — usually not the OS default) — an audio-only publisher for external mics. The device reports its `kind` (`camera`/`screen`/`audio`) on register so the operator routes it to the right slot. No control-service change to publish — every kind is just a publisher.
...
- **Crossfade transitions** (optional, `POST /api/export?id=&crossfade=1&fade=`). When off (default), the normalized parts are hard-cut **concatenated with stream-copy** (fast). When on, a final **re-encode pass** chains `xfade` (video) + `acrossfade` (audio) over the parts, so consecutive segments dissolve; the program shrinks by `(N-1)*fade`. `planXfade` (pure, unit-tested) computes the per-transition offsets and **clamps the fade to the shortest part**, bailing to hard cuts if no fade fits. Fade duration is operator-chosen (0.3/0.5/1.0s) in the dashboard's export dialog.
...
- **Switch-log preview player**: each session has a **Preview** that plays the program edit in-browser — one active angle at a time, swapping at each logged take, with a scrubber and take markers. Each per-angle clip is mapped onto the session timeline via its `recordStartedAt`; pre-roll shows a black "before first take" panel, a take whose camera has no file shows "no footage", and a take whose clip's footage **runs out before the take ends** swaps from the (misleading) frozen frame to a "footage ended" panel — the timeline keeps advancing and ends cleanly at `durationSec`. It's a synced single-active-`<video>` preview (not a rendered export), so cross-clip sync is approximate, not frame-accurate. A spinner overlays the stage whenever the active clip can't play yet (buffering or seeking — driven by the video's load/seek events, so it also shows on initial load and while paused). The scrubber supports **drag-scrubbing** (pointer capture, so the fill and frame follow while dragging, not only on release) and a **Fullscreen** button (the stage grows to fill the screen; controls stay visible). Hovering the scrubber shows a **thumbnail preview**: a hidden per-clip `<video>` is seeked to the hovered time and its frame drawn to a canvas tooltip (seeks are coalesced to the latest hover position; a small spinner appears in the tooltip during a slow seek); black gaps show a text label instead. At each take the angles **cross-dissolve** (an approximation of the export's crossfade): the video layers are opacity-blended over ~0.3s during playback, switched instantly on seek/scrub. The buffering spinner is delayed ~220ms so the brief clip-swap seek under a dissolve doesn't flash it. This is approximate — it always dissolves and uses a fixed duration, regardless of the export's actual crossfade choice.
...
- **Audio sources panel**: external mics (`kind:'audio'`, path `micN`) are managed separately from cameras — add/rename/remove, a **link-to-camera** dropdown (the export's per-section routing target), and a **Listen** monitor (subscribes to the mic's WHEP audio so the operator can hear it). Audio sources have no video tile, aren't takeable as a program angle, and the pre-flight checks them for an audio track (not H.264 video). Audio-kind devices in the roster assign only to mic slots; camera/screen devices only to camera slots.
...
- **Configurable bitrate**: done — operator-controlled global target (`data/settings.json`) plus per-camera overrides (`cameras.json`), surfaced as a header Bitrate selector and a per-camera dropdown. Effective bitrate rides in the publish command and live `{type:'bitrate'}` messages so phones adjust mid-session via `setParameters` (no renegotiation).
- **Screen as a camera** (#3): done — the device page adds a capture-source selector (Camera / Screen share); screen uses `getDisplayMedia` (feature-detected: desktop + Android Chrome, not iOS), publishes H.264 through the same WHIP pipeline, and is just another publisher to the control service (no server change). Validated client-side via the proxy-repro trick (source toggle, screen-join state, "stop sharing" handling); the WHIP/record path is the same one already proven with real devices. User-facing copy renamed "phone" → "device" (dashboard roster, tray menu, device page); wire protocol unchanged. Effective bitrate rides in the publish command and live `{type:'bitrate'}` messages so phones adjust mid-session via `setParameters` (no renegotiation). Control-service flow covered by `npm test`; dashboard validated via the proxy-repro trick (correct messages, header re-sync, no layout overflow).
...
- **External mic audio (#4)**: done.
  - *Phase 1 (capture + record):* the device page's *Audio only (mic)* source (with mic picker) publishes audio-only; the control service models it as a `kind:'audio'` source on a `micN` path (recorded as its own clip, persisted, kept out of tiles/takes/audio-only pre-flight); the dashboard has an Audio sources panel (add/rename/remove, link-to-camera, **Listen** monitor) and routes audio devices to mic slots. **Listen** makes that one device publish on its own (`monitorPublish` — doesn't flip global preview) then subscribes to its WHEP audio, and auto-reverts if the source stops being live; a connected mic device can also spin up its own source (create + assign) in one click (`addAudioSource{assign}`). The finalized session records each source's `kind`+`link`+`recordStartedAt`. Individual mic clips remain separate files for post.
  - *Phase 2 (per-section routing + preview mixer):* each program section can route a mic — **mix** (camera + mic) or **replace** (mic only) — with camera/mic volume sliders, edited by **clicking the section in the preview** (live: the camera `<video>` volume + a synced mic `<audio>` play the mix as you tune it; element volumes cap at 100%). Config persists per session (`POST /api/sessions/audio` → `audioRouting`), defaulting to the section camera's linked mic. The export renders the same routing: `sectionAudio` resolves override→link→camera-only, and each part bakes in a `volume`+`amix` graph (mute camera bed for replace) so it composes with concat/crossfade. `npm test` (35) covers `sectionAudio`/`planParts`/`setSessionAudio`; the real-ffmpeg pass verified mix is ~3 dB over replace/camera-only; the preview editor + persistence + live levels validated via the proxy-repro trick.
- **Crossfade transitions** (#2): done — optional `xfade`/`acrossfade` dissolves in the export (opt-in via the dashboard's export dialog: a checkbox + 0.3/0.5/1.0s duration; default stays hard-cut copy-concat). The preview approximates it with an opacity cross-fade between video layers. `planXfade` offset/clamp logic covered by `npm test` (31); the real ffmpeg pass validated end-to-end (hard-cut 6.07s vs crossfade 5.08s — shrinks by `(N-1)*fade`); dialog + preview dissolve validated via the proxy-repro trick.
...
### Known limitations
- The cross-platform Node launcher (`cli/`) is end-to-end validated **on Windows only**. The macOS/Linux paths are *code-audited* — every tool download URL (mkcert / MediaMTX / BtbN-ffmpeg / evermeet-ffmpeg, for darwin amd64+arm64 and linux amd64+arm64) is HEAD-checked and resolves; `down()` falls back from `lsof` to `ss -ltnpH` so a minimal Linux without lsof still works; `virbr` (libvirt bridge) is skipped in LAN-IP detection. Not run end-to-end on those OSes.
- **Single-exe build** is verified on Windows only; the macOS/Linux `build:exe` paths are code-audited (correctly skips the PE icon stamp off-Windows, sets the Mach-O segment name on macOS via postject, chmods 0755 on the finished binary) but not run end-to-end on those OSes. macOS ffmpeg is Intel-only (evermeet); on Apple Silicon it runs under Rosetta 2 — works, but suboptimal. The injected exe trips an "Authenticode signature corrupted" warning (expected — `postject` appends a resource to the signed `node` binary); it still runs. The exe is not standalone — it needs a working directory with `tools/` and the asset/writable dirs (see [Single-exe build](#single-exe-build-no-node-install-to-run)).
- **Session export (v1)** re-encodes to a fixed **1080p30** H.264 + AAC and uses **one camera per program segment** (optional crossfade dissolves at takes, plus per-section external-mic routing — mix/replace + volumes — but no other transitions, no auto-ducking, no per-export resolution/bitrate choice). Cross-clip/clip-vs-mic sync is **approximate** (sub-second), matching the preview — not frame-accurate. ffmpeg is pulled from BtbN's rolling `latest` build (Windows/Linux) and evermeet (macOS, Intel — runs under Rosetta on Apple Silicon); only the Windows download path is verified.

## Roadmap (planned)

Tracked as GitHub issues so contributors can see what's intended:

- **[macOS & Linux support](https://github.com/openidle-dev/idle-stream/issues/1)** (#1) — core runtime is cross-platform (Node + MediaMTX/mkcert/ffmpeg, all of which ship Mac/Linux builds) and the launcher/SEA-build paths for those OSes are now code-audited: download URLs HEAD-verified, `down()` has an `ss` fallback when `lsof` is absent, libvirt's `virbr` is skipped in LAN-IP detection, the SEA build correctly skips the Windows-only PE icon stamp and sets the Mach-O segment name on macOS. Outstanding is *running* it on real Mac/Linux hardware. Bigger remaining work: per-OS packaging (macOS `.dmg`/`.app`, Linux AppImage/`.deb`), a per-OS window-free **tray** (macOS menubar, Linux libappindicator) to replace the Windows hidden-PowerShell tray, native ARM64 ffmpeg on Apple Silicon (avoid Rosetta), plus macOS code-signing/notarization. Architecture doesn't block any of it.
- **[Crossfade transitions in the export](https://github.com/openidle-dev/idle-stream/issues/2)** (#2) — **done** (see [Build Status](#build-status)): optional `xfade`/`acrossfade` dissolves in the export (opt-in dialog, 0.3/0.5/1.0s), with the preview approximating it via an opacity cross-fade.
